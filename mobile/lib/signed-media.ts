...
const LOCAL_URI_PREFIXES = ["file:", "content:", "blob:", "data:"];

const isLocalUri = (value: string) =>
  LOCAL_URI_PREFIXES.some((prefix) => value.startsWith(prefix));

const extractStorageKey = (rawUrl: string) => {
  const index = rawUrl.indexOf(STORAGE_MARKER);
  if (index === -1) return null;
  const keyWithQuery = rawUrl.slice(index + STORAGE_MARKER.length);
  return keyWithQuery.split("?")[0];
};

const readCache = (storageKey: string) => {
